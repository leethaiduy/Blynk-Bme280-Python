...
#Blynk Setup
BLYNK_AUTH="IWcZQozlHCTf-QTBqz6l65DxEq_32CPU"
BLYNK_SERVER ="duyle.zapto.org"
BLYNK_PORT = 9444
...
def readBME280ID(bus,addr=DEVICE):
...
def main(bus,blynk):
  ...
  # (chip_id, chip_version) = readBME280ID(bus)
...
if __name__=="__main__":
  while True:
    try:
      ...
      with open(os.path.join(os.path.dirname(__file__), "log"), "a") as logf:
        now = datetime.now()
        dt_string = now.strftime("%d/%m/%Y %H:%M:%S")
        logf.write("{0}: {1}\n".format(str(dt_string), str(e)))
        time.sleep(10)
